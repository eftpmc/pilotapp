...
export function seedTask(userId: string, projectId: string, overrides: Partial<{ title: string; prompt: string; status: string; priority: number; agentId: string; parentTaskId: string }> = {}) {
  const id  = uuid();
  const now = new Date().toISOString();
  ...
  if (overrides.parentTaskId) db.prepare('UPDATE tasks SET parent_task_id = ? WHERE id = ?').run(overrides.parentTaskId, id);
  return { id };
}
